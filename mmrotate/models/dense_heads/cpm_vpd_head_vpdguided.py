...
class CPMVPDHead(CPMHead):
    ...
    def __init__(self, *args,
        ...
        self.assign_transition_end = int(train_cfg.get('assign_transition_end', 5000))
        # -----------------------------------
    ...
    def _get_target_single_vpd(self, gt_bboxes, gt_labels, bbox_preds_single, points, regress_ranges, strides, num_points_per_lvl):
        """Like CPMHead._get_target_single but also returns gt_instance_ids and 
        supports VPD-calibrated distance during later training stages.
        """
        alpha = self.alpha
        ...
        if num_gts == 0:
            return (gt_labels.new_full((num_points,), self.num_classes), gt_ids)
            
        dist_sample_and_gt = torch.cdist(points, center_point_gt)  # (P, G) 原始几何距离

        # --- 修改开始: VPD 预测校准距离混合 ---
        # 1. 设定过渡期超参 (可自行调整或移至 __init__)
        ...
        if w_vpd > 0.0 and bbox_preds_single is not None:
            # bbox_preds_single[:, :2] 是 stride-normalized 的 mu
            # 乘以 strides 恢复为图像尺度的偏移量
            delta_pred = bbox_preds_single[:, :2] * strides.unsqueeze(1)
            pred_centers = points + delta_pred
            dist_pred_to_gt = torch.cdist(pred_centers, center_point_gt)  # (P, G) 校准距离
            # 线性插值混合
        ...
        # Additional background labels (保持使用原始几何距离，因为是针对点本身的空间位置)
    ...
    def get_targets_vpd(self, points, gt_bboxes_list, gt_labels_list, bbox_preds_list=None):
        ...
        labels_list, gt_ids_list = multi_apply(
            ...
            gt_bboxes_list,
            gt_labels_list,
            bbox_preds_list,  # 传入预测偏移
            points=concat_points,
            regress_ranges=concat_regress_ranges,
            strides=concat_strides,  # 传入步长
    ...
    def loss(self, cls_scores, bbox_preds, centernesses, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        # flatten_bbox_preds_per_lvl: list of (N, P_lvl, 4)
        ...
        # 修改后：在初步候选池上计算 VPD loss (例如满足几何距离 < thresh1 的点)
        # 这样能保证 VPD 在动态分配前具备基本的预测能力
        candidate_inds = ((flatten_labels >= 0) & (flatten_labels < bg_class_ind)).nonzero().reshape(-1) # 或者使用更宽泛的几何距离条件
        pos_bbox_preds = flatten_bbox_preds[candidate_inds]
        
        pos_points = flatten_points[pos_inds]
        pos_img_ids = img_ids[pos_inds]
        pos_gt_ids = flatten_gt_ids[pos_inds]
        pos_strides = flatten_strides[pos_inds]
        bbox_mu = pos_bbox_preds[:, :2]
        bbox_log_sigma = pos_bbox_preds[:, 2:]

        gt_centers_per_pos = torch.zeros(len(pos_inds), 2, device=bbox_preds[0].device)
        ...
        vpd_losses = self.loss_vpd(
            bbox_mu=bbox_mu, bbox_log_sigma=bbox_log_sigma, pos_points=pos_points,
            pos_strides=pos_strides, gt_centers=gt_centers_per_pos,
            gt_centers_list=gt_centers_list, cur_iter=self.iter,
            pos_img_ids=pos_img_ids, num_samples=self.num_samples_train)
