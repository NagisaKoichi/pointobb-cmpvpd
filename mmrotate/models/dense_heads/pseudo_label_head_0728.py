...
class PseudoLabelHead(RotatedFCOSHead):
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, cls_scores_all, img_metas, points, regress_ranges, num_points_per_lvl, alpha=1, thresh1=8, thresh2_bg=4, thresh3=0.1, pca_length=28, default_max_length=128, mode='near'):
        ...
        Args:
            cls_scores_all (list[Tensor]): feature map scores on each point for each scale level, len(cls_scores_all) = num_levels, cls_scores_all[i].size() = (num_classes, w, h)
        """
        alpha = alpha
        ...
        # === 步骤 1：构造 Level 1 的上采样版本，用于方向计算 (PCA) ===
        # 取出 Level 1 和 Level 0
        lvl1_feat = cls_scores_all[1]
        lvl0_feat = cls_scores_all[0]
        target_h, target_w = lvl0_feat.shape[1], lvl0_feat.shape[2]
        
        # 将 Level 1 上采样到 Level 0 的尺寸
        lvl1_feat_up = F.interpolate(
            lvl1_feat.unsqueeze(0), 
            size=(target_h, target_w), 
            mode='bilinear', 
            align_corners=False
        ).squeeze(0)
        
        # 构造临时列表，索引0位置存放上采样后的 Level 1
        cls_scores_for_pca = list(cls_scores_all)
        cls_scores_for_pca[0] = lvl1_feat_up

        # === 步骤 2：方向计算 (PCA) - 使用上采样后的 Level 1 ===
        # 传入 cls_scores_for_pca，函数内部会取索引0（即上采样后的L1）
        center_factor = self.get_center_factor(center_point_gt, gt_labels, cls_scores_for_pca[0])
        gt_ctr_rect = self.get_rectangle_cls_prob(cls_scores_for_pca[0].sigmoid(), self.strides[0], center_factor, center_point_gt, pca_length, mode='near')
        ...
        # === 步骤 3：边界搜索 - 使用原始 Level 0 ===
        # 原始的 get_edge_boundary_simple 内部硬编码了使用 cls_scores_all[0] 和 strides[0]，
        # 所以我们直接传入原始的 cls_scores_all 即可。
        # 注意：这里不传入 target_level 参数。
        top_simple, bottom_simple = self.get_edge_boundary_simple(
            gt_labels, eigvec_first, center_point_gt, 
            cls_scores_all,  # 传入原始列表，函数内部会自动取用 Level 0
        ...
        left_simple, right_simple = self.get_edge_boundary_simple(
            gt_labels, eigvec_second, center_point_gt, 
            cls_scores_all, # 传入原始列表
            thresh3, default_max_length, mode='simple', 
            is_secondary=True, 
            is_nearest_same_class=is_nearest_same_class, 
            nearest_gt_point=center_point_gt[dist_min_gt_and_gt_index]
