...
def _get_gt_nearest_gt_dist(cx, cy, img_meta, k=4):
    """
    Get the distance from each GT center to k-th nearest GT center.
    Args:
        cx (Tensor): shape [n_gt], GT center x coordinates in image
        cy (Tensor): shape [n_gt], GT center y coordinates in image
        img_meta (dict): image meta info containing original image shape
        k (int): which nearest neighbor distance to return (e.g. k=4 for 4th nearest)
    Returns:
        Tensor: shape [n_gt], distance to k-th nearest GT center in image pixels
    """
    n_gt = cx.shape[0]
    if n_gt <= 1:
        return torch.full((n_gt,), float('inf'), device=cx.device)

    img_h, img_w = _infer_image_shape(img_meta)
    centers = torch.stack([cx, cy], dim=1)  # shape [n_gt, 2]
    dists = torch.cdist(centers, centers)  # shape [n_gt, n_gt]
    dists.fill_diagonal_(float('inf'))  # ignore self-distance
    knn_dists, _ = torch.topk(dists, k=k, dim=1, largest=False)
    kth_dists = knn_dists[:, -1]  # shape [n_gt]
    return kth_dists
...
def _compute_gt_guided_maps(p_model,
    ...
    # this step uses gt_bbox size to generate sigma, which is banned in PSOD tasks.
    # sigma = sigma_scale * torch.sqrt((bw_f * bh_f).clamp(min=1e-6))  # shape [n_gt]
    
    # use knn distance to GT centers to generate sigma, as an alternative GT-guided map for VPD.
    knn_dists = _get_gt_nearest_gt_dist(cx_f, cy_f, img_meta, k=1)  # shape [n_gt, K]
    sigma = sigma_scale * knn_dists[0]
